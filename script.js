...
window.downloadImage = function () {
    ...
    const originalClasses = captureEl.className;
    const homeIndicator = document.querySelector('.absolute.bottom-2.left-1\\/2'); // Home bar

    // 1. Remove Phone Styling for "Screenshot" Look (No rounded corners, no border)
    // We want to keep the width/height but remove the outer bezel look
    captureEl.classList.remove('border-[8px]', 'border-gray-900', 'rounded-[40px]', 'shadow-2xl');

    // Hide Home Indicator for a cleaner specific chat view or keep it? 
    // Usually screenshots HAVE the home bar. I will keep it.

    chatContainer.style.overflow = 'hidden';

    // 4. Temporary Styles for Capture
    const truncateElements = document.querySelectorAll('.truncate');
    truncateElements.forEach(el => {
        el.classList.remove('truncate');
        el.style.whiteSpace = 'nowrap'; // Keep on one line but allow overflow
        el.style.overflow = 'visible';
    });

    // Fix "Online" status cutting off by ensuring parent is visible
    const statusEl = document.getElementById('previewStatus');
    if (statusEl) {
        statusEl.parentElement.style.overflow = 'visible';
        statusEl.parentElement.style.minWidth = '200px'; // Force width?
    }

    html2canvas(captureEl, {
        scale: 3, // Increased scale for better quality
        useCORS: true, // Critical for external images
        allowTaint: false, // Must be false for dataURL export
        backgroundColor: null,
        logging: true,
        onclone: (clonedDoc) => {
            // Further manipulation of the cloned DOM if needed
            const clonedChat = clonedDoc.getElementById('chatContainer');
            if (clonedChat) clonedChat.style.overflow = 'visible';
        }
    }).then(canvas => {
        // Restore styles
        captureEl.className = originalClasses;
        truncateElements.forEach(el => {
        ...
        if (statusEl) {
            statusEl.parentElement.style.overflow = 'hidden';
            statusEl.parentElement.style.minWidth = '';
        }
